...
| Place and route | `make pnr` | Full RTL-to-GDS through LibreLane, both variants: post-route area, post-route timing at all three corners with extracted parasitics, router and Magic and KLayout DRC, a Magic-against-KLayout GDS XOR, and netgen LVS. Both pass LVS with no unmatched net, device or pin. The folded variant is DRC clean; the pipelined variant is not, and the counts are reported rather than rounded away. |
| Formal | `make formal` | Attempted and **not established**. A SymbiYosys miter for variant equivalence and a set of OBI protocol properties are both written and both elaborate, and the solver runs out of time on every configuration tried. [formal/README.md](../formal/README.md) records each attempt with its bound and its wall clock. Nothing in this repository rests on a formal result. |
